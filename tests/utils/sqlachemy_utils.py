...
# see https://github.com/kvesteri/sqlalchemy-utils/issues/611#issuecomment-1448480978


async def _get_scalar_result(engine: sa.Engine, sql):
    try:
...
async def create_database(url, encoding="utf8", template=None):
    ...
    if (
        ...
        and dialect_driver
        in {"asyncpg", "pg8000", "psycopg2", "psycopg2cffi"}
    ):
